...
//echo document->getElementById('one-eight')->innerHTML;
...
if(isset($_POST["mng"]))
{
  switch ($_POST["mng"])
  {
    case 'AddWork':
      ...
      if ($result)
      {
        if (!empty($_POST["favorite"]))
        {
          ...
          $doc->getElementById('addFav_wID')->setAttribute("value",$wID);
          //$doc->getElementById('update_fd')->setAttribute("style","zIndex:1");
          popUpForm('update_fd');
        }
        else
        {
    ...
    case 'Search':
      ...
      $result=$work->getWorkById($_POST["wID"]);
    ...
    case "Remove":
      ...
      if($result2->num_rows > 0)
      {
        ...
        if($result3)
        {
          $dvMsg2->nodeValue="Remove Favorite Successfully!";
        }
        else
        {
      ...
      else
      {
        ...
        if($result4)
        {
          $dvMsg2->nodeValue="Remove Work Successfully!";
        }
        else
        {
          $dvMsg2->nodeValue="Remove Work Failed!";
        }
      }
...
function showWork($result)
{
  ...
  while($row = $result->fetch_assoc())
  {
    ...
    //$imgNode->setAttribute("id",$row["Name"]);
    $imgNode->setAttribute("alt",$row["Name"]);
